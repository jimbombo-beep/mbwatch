...
public class Watch extends Group implements EventHandler<Event> {
	...
	private Node _create24HourLabel(int n) {
		...
		trans.setX(_outerR + 0.75 * _outerR * Math.cos(rad) - 0.05 * _outerR);
		trans.setY(_outerR + 0.75 * _outerR * Math.sin(rad) + 0.05 * _outerR);
	...
	private Node _create24HourLabels() {
		...
		Circle c = new Circle(0.6 * _outerR);
	...
	private Node _createMinuteLabel(int n) {
		double rad = Math.toRadians(30 * n - 90);
		Translate trans = new Translate();
		trans.setX(_outerR + 0.5 * _outerR * Math.cos(rad) - 0.05 * _outerR);
		trans.setY(_outerR + 0.5 * _outerR * Math.sin(rad) + 0.05 * _outerR);
	...
	private Node _createTickMark(int n) {
		...
		l.setEndY(_outerR * (n % 3 == 0 ? 0.225 : 0.175));
		l.getTransforms().add(rot);
		l.setStrokeWidth((n % 3 == 0 ? 3.0 : 2.0));
