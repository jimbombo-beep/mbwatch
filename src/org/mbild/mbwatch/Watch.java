...
public class Watch extends Group implements EventHandler<Event> {
	...
	private Node _createDate() {
		...
		lblDay.textProperty().bind(_clockwork.day);
		
		Label lblSep1 = new Label(".");
		
		Label lblMonth = new Label();
		lblMonth.textProperty().bind(_clockwork.month);
		
		Label lblSep2 = new Label(".");
		
		Label lblYear = new Label();
		lblYear.textProperty().bind(_clockwork.year);
		
		GridPane grid = new GridPane();
		grid.getStylesheets().add(Configuration.CSS);
		grid.getStyleClass().add("date-label");
		grid.add(lblDay, 0, 0);
		grid.add(lblSep1, 1, 0);
		grid.add(lblMonth, 2, 0);
		grid.add(lblSep2, 3, 0);
		grid.add(lblYear, 4, 0);
		
		grid.layout();
		
		Translate trans = new Translate();
		trans.setX(_outerR - 0.3 * _outerR);
		trans.setY(_outerR * 1.35);
		
		grid.getTransforms().add(trans);
		grid.toFront();
