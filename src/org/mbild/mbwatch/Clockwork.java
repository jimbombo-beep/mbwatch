...
public class Clockwork {
    ...
    public Clockwork(String timeZone) {
    	_timeZone = timeZone;
    	_date = LocalDate.now(ZoneId.of(timeZone));
    	_time = LocalTime.now(ZoneId.of(timeZone));
    ...
    private EventHandler<ActionEvent> _updateTime() {
        return event -> {
        	...
        	LocalDate tmpDate = ldt.toLocalDate();
        	LocalTime tmpTime = ldt.toLocalTime();
        	
        	fireFXTimerEvent(new FXTimerEvent(FXTimerEvent.UPDATE_TIME, _timer));
        	
        	if(!tmpDate.isEqual(_date))
        		fireFXTimerEvent(new FXTimerEvent(FXTimerEvent.UPDATE_DATE, _timer));
